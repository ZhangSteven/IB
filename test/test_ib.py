...
class TestIB(unittest2.TestCase):
    ...
    def testTradeRecords(self):
        records = list(createTradeRecords(join(get_current_path(), 'samples', 'trade1',
            'DU1237908.Trades_TradeConfirmFlex.Sample.csv')))
        self.assertEqual(len(records), 32)
        self.verifyTrade1(records[0])
        self.verifyTrade2(records[30])
    ...
    def verifyTrade1(self, record):
        ...
        self.assertEqual(datetime(2018,10,23), record['SettlementDate'])
    ...
    def verifyTrade2(self, record):
        ...
        self.assertEqual('S H9 Comdty', record['BloombergTicker'])
        self.assertEqual('Short', record['Side'])
        self.assertEqual(15, record['Quantity'])
        self.assertAlmostEqual(883.75, record['Price'])
        self.assertEqual(datetime(2018,10,22), record['TradeDate'])
        self.assertEqual(datetime(2018,10,23), record['SettlementDate'])
